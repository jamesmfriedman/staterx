...
export var createStore = function (observables, initialState) {
  ...
  var stateRxBranches = Object.values(observables).filter(function (branch) {
    return 'state$' in branch;
  });
  ...
  var reducer$ = merge.apply(void 0, normalizedObservables).pipe(
  ...
  var action$ = merge
    .apply(
      ...
      __spreadArrays(
        [dispatch$],
        stateRxBranches.map(function (state) {
          return state.action$;
        })
      )
    )
  ...
  var setState = function (newState) {
    for (var key in newState) {
      ...
      if ('state$' in obs) {
        ...
        console.warn(
          'Cannot set store value of ' +
            key +
            '. It is not a StateRx Objects or subject.'
        );
      }
    }
  };
