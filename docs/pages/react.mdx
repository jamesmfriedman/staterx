...
StateRx has built in React support through hooks. The `useRx` and `useRxMemo` hooks actually work with ANY Observable, not just StateRx Objects. The following example creates a value and consumes both the `state$` and the custom `callCount$` Observable.
...
`useRx` takes two arguments, an Observable and an optional default value. Similar to React's `useState`, it returns an array. The first item in the array is the value, however, the second value is actually an error in the event that our observable has an unrecoverable error.

**Attention** Since Observables don't always have default values, be aware that your state's value might initially be `undefined` on the first render since subscriptions in RxJS are async. Typescript will alert you of this potential issue, but for non Typescript users be aware that passing a default value is always a safe idea.
...
There are plenty of situations where consuming an effect requires passing some parameters to create an Observable. A basic use case is selecting an item by id. For these situations you should use the `useRxMemo` hook. This hook takes a function that returns an Observable, a list of dependencies, and an optional default value.

The example below uses `createItems` to create a list of todos. `createItems` comes with a bunch of useful built in effects, in this case we're going to use the `byId` effect. The example below is editable, you can modify the id and `useRxMemo` will respond accordingly.
...
<Playground>{() => {
  const id = '1';
  const [todo] = useRxMemo(() => todos.byId(id), [id]);
